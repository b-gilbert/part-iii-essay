...
# - b the value of the non-zero coefficients of beta
# - sigma_e the scalar for for the covariance matrix of the rows of E
# - vartheta the scalar for the covariance matrix of the rows of Gamma
generate_data_mod2 <- function(n=300, p=400, r=6, s=5, b=1, sigma_e=1, vartheta=1){
  ...
  # sample the rows of gamma from a N(O, kappa*Ip) distribution
  gamma = matrix(rnorm(r*p, mean=0, sd=vartheta), r, p)
...
h = 1000
span = 10^(seq(-5,0,1/10))

for (vartheta in span){
  print(paste("Beginning: vartheta=", as.character(vartheta), sep=""))
  ...
  for (i in 1:h){
    ...
    #data generation
    data <- generate_data_mod2(n=n, p=p, r=r, s=s, b=b, sigma_e=sigma_e, vartheta=vartheta)
...
# plot
png(file=paste(EXPORT_PATH, "fig6b.png", sep=""), width=500, height=450)
plot(x=span, y=lasso_errors, type="l", lty=5, lwd=2, xaxt="n", log="x", xlab=expression(vartheta), ylab="L1 estimation error", col="gray", xlim=c(10^-3, 10^0), ylim=c(0, 5))
axis(1, at = 10^(seq(-3,0,1)), las=0)
...
abline(v = sqrt((1/600)*sqrt(1/2)), col="tomato3", lwd=2, lty=1)
