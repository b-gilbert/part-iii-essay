...
for (p in span){
  ...
  for (i in 1:h){
    ...
    #data generation
    data <- generate_data(n=n, p=p, r=r, s=s, b=b, sigma_e=sigma_e)
    ...
    for (method in c("LASSO", "Oracle PCA", "Trim")){
      ...
      # lasso regression of X_tilde on Y_tilde (lambda from cross validation)
      data$reg <- glmnet(X_tilde, Y_tilde, alpha=1, lambda.min.ratio=0.1, nlambda=100)
      data$lambda <- cv.glmnet(X_tilde, Y_tilde, alpha=1, lambda=data$reg$lambda, nfolds=10)$lambda.min
      data$reg <- glmnet(X_tilde, Y_tilde, alpha=1, lambda=data$lambda)
...
# plot
png(file=paste(EXPORT_PATH, "fig5b.png", sep=""), width=500, height=450)
plot(x=span, y=lasso_errors, type="l", lty=5, lwd=2, xlab="Number of covariates (p)", ylab="L1 estimation error", col="gray", xlim=c(0, 600), ylim=c(0, 3))
